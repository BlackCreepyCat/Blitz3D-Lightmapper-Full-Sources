...
; Assigns a 2nd channel planar mapping coordinates to the mesh and returns a packed texture 
; that can be applied for lightmapping
;
; NOTES: 
;
...
Function LMExample()
	...
	cube3 = CreateSphere(10)
	PositionMesh(cube3, 4, 4,4) 
	ScaleMesh(cube3, 4, 2, 2)
		...
		If MouseHit(1)
			...
			If ent
				...
				If EntityName(ent) = "cube1"
					tex = LightMapMesh(ent, 0.2, 512, 1, "Lightmapping " + EntityName(ent))
				Else
					tex = LightMapMesh(ent, 0.02, 512, 1, "Lightmapping " + EntityName(ent))	
				EndIf
